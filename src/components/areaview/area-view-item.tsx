    ...
    return (
      <li
        ...
        <div style={itemStyles}>
          <span style={item.done ? itemDoneStyles : {}}>{item.name}</span>
          {
            <div style={itemExtraStyles}>
              {item.inPrimaryList && (
                <span style={mix(extraDirectChild, { color: "blue" })}>
                  {this.props.isPrimary ? item.source : "我的一天"}
                </span>
              )}
              {item.comments && <span style={extraDirectChild}>备注</span>}
            </div>
          }
        </div>
      </li>
